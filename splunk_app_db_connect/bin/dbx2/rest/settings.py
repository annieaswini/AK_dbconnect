...
class Settings(SimpleRest):
    ...
    def update_dbx_java_home(self, javaHome):
        ...
        java_path_linux64 = os.path.join(app_dir, "bin", self.customized_java_path)
    ...
    def restart_task_server(self, taskserver_port):
        try:
            ...
            logger.debug("restart_task_server url is {}".format(self.restart_url % taskserver_port))
            requests.delete(self.restart_url % taskserver_port, verify=False)  # nosemgrep - internal DBX Task Server call to delete old Task Server IP:Port
        except Exception as ex:
