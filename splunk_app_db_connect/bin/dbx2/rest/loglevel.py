...
class LogLevelHandler(SimpleRest):
    ...
    def _update_server_log_level_runtime(self, server_log_level):
        ...
        try:
            ...
            verify_ssl = not self._is_localhost(self.server_log_level_url)
            response = requests.put(url=self.server_log_level_url, headers=headers, data=dumps(data), verify=verify_ssl)
