...
import platform
import subprocess
from dbx2.splunk_client.api_config import ApiConfig
...
class HealthChecker(SimpleRest):
    ...
    path_to_app = os.path.join(splunk_home, 'etc', 'apps',
                               'splunk_app_db_connect')
    path_to_checkpoint = os.path.join(splunk_home, 'var', 'lib', 'splunk',
                                      "modinputs", "server",
                                      "splunk_app_db_connect")
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.splunk_scheme = ApiConfig.splunkd_scheme()
        self.splunk_host = ApiConfig.splunkd_host_port()[0]
        self.splunk_port = ApiConfig.splunkd_host_port()[1]
    ...
    def illegalAction(self, verb):
        ...
        self.addMessage('ERROR',
                        'HTTP %s not supported by the settings handler' % verb,
                        405)
    ...
    def handle_GET(self):
        try:
            ...
            if item_to_check not in self.check_dispatcher:
                raise Exception(
                    "health check item is invalid, only the following items are supported: {}".format(
            ...
            self.response.setHeader('content-type', 'application/json')
            self.return_json["entry"]["content"][
                "severity_level"] = Status.ERROR
    ...
    def _check_java_installation_health(self):
        ...
        if not os.path.isfile(path_to_customized_java):
            ...
            if is_healthy:
                splunk_service = SplunkServiceFactory.create(self.sessionKey,
    ...
    def _check_java_server_configuration_health(self):
        ...
        input_url = "{}://{}:{}/services/data/modular-inputs/server".format(
            self.splunk_scheme, self.splunk_host,
            self.splunk_port)
        logger.debug("_check_java_server_configuration_health url is {}".format(
            input_url))

        try:
            verify_ssl = not self._is_localhost(input_url)
            response = requests.get(url=input_url, headers={
    ...
    def _check_commands(self, splunk_service):
        ...
        error_message = "{} command conf's filename property should be {}, but it is: {} now."
        command_default_filename = "java.path"
        commands_endpoint = "configs/conf-commands/{}"
        commands = ["dbxoutput", "dbxlookup"]
        for command in commands:
            filename = client.Entity(splunk_service,
                                     commands_endpoint.format(command)).content[
                "filename"]
            if filename != "java.path":
                current_command_check = (
                    Status.ERROR,
                    error_message.format(command, command_default_filename,
                                         filename))
            else:
                current_command_check = (
                    Status.OK,
        ...
        # The dbxquery config has changed, default value of filename is different from other commands.
        command = "dbxquery"
        dbxquery_default_filename = "dbxquery_bridge.py"
        filename = client.Entity(splunk_service,
                                 commands_endpoint.format(command)).content[
            "filename"]
        if filename != dbxquery_default_filename:
            dbxquery_command_check = (
                Status.ERROR,
                error_message.format(command, dbxquery_default_filename,
                                     filename))
        else:
            dbxquery_command_check = (
                Status.OK,
    ...
    def _check_file_permission_health(self):
        unhealthy_folders = []
        paths_to_check_permission = [self.path_to_app, self.path_to_checkpoint,
                                     self.path_to_log]
        for path in paths_to_check_permission:
            has_read_and_write_permission = (
                os.access(path, os.R_OK) and os.access(path, os.W_OK))
            logger.debug("file: {}, has read/write permission: {}".format(path,
                                                                          has_read_and_write_permission))
    ...
    def _forward_request_to_proxy(self):
        ...
        forward_url = "{}://{}:{}/servicesNS/{}/splunk_app_db_connect/{}/dbxproxy/{}" \
            .format(self.splunk_scheme, self.splunk_host, self.splunk_port,
                    self.userName.replace(" ", "%20"), self.pathParts[1],
                    "/".join(self.pathParts[2:]))
        logger.debug("_forward_request_to_proxy url is {}".format(forward_url))
        try:
            verify_ssl = not self._is_localhost(forward_url)
            response = requests.get(url=forward_url, headers={
                'Authorization': ('Splunk %s' % self.sessionKey)},
                                    verify=verify_ssl)
            if response.status_code != 200:
                logger.warn(
                ...
                raise Exception(
                    "fail to get health check result from server url={} status_code={} content={}".format(
                        forward_url,
    ...
    def _check_java_server_availability(self):
        taskserver_status_url = "{}://{}:{}/servicesNS/{}/splunk_app_db_connect/db_connect/dbxproxy/taskserver" \
            .format(self.splunk_scheme, self.splunk_host, self.splunk_port,
                    self.userName.replace(" ", "%20"))
        logger.debug("_check_java_server_availability url is {}".format(
            taskserver_status_url))
        status = Status.OK
        detail = ''
        try:
            verify_ssl = not self._is_localhost(taskserver_status_url)
            ...
            if response.status_code != 200:
                logger.error(
                    "action=check_java_server_availability_failed error={}".format(
                        taskserver_status_url,
                        response.content))
                status = Status.ERROR
    ...
    def _execute(self, cmd):
        ...
        if rc != 0:
            logger.error(
                "action=failed_to_execute_command:{} error={}".format(cmd,
                                                                      error))
        return rc, result
