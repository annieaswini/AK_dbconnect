...
def write(jvm_options, filename):
    ...
    with open(jvm_options_file_path, "w") as vmopts_file:
        logger.debug("action=write_jvm_options_to_file, filepath={}jvmopts={}".format(jvm_options_file_path, jvm_options))
...
def get_property(jvmopts, property, property_regex):
    ...
    if matched:
        logger.debug("action=read_property_from_jvmoptions, jvmoptions={} property={} property_regex={}".format(jvmopts, property, property_regex))
        return matched.group(1)
    else:
        logger.debug("action=fail_to_read_property_from_jvmoptions, jvmoptions={} property={} property_regex={}".format(jvmopts, property, property_regex))
        return None
